            ...
            document.addEventListener('click', (e) => {
                if(e.target.classList.contains('slide-save')) {
                    ...
                    editor.save().then((data) => {
                        ...
                        props.data.text = JSON.parse(sessionStorage.getItem(slideName)).blocks[0].data.text
        ...
        <form className="uk-grid uk-grid-medium" data-uk-grid>
            <div className="uk-width-expand">
                <div className="uk-inline-clip component-slide-new uk-flex uk-flex-left uk-flex-middle uk-background-contain uk-background-center-left" style={{backgroundImage: 'url("assets/img/blockit-slide-code.svg")'}}>
                    <span className="uk-label uk-label-warning uk-text-small uk-border-rounded component-slide-count">id : {props.data.id}</span>
                    <div className="uk-position-cover uk-flex uk-flex-right uk-flex-middle">
                        <button className="uk-button uk-button-secondary uk-border-rounded uk-margin-small-right" type="button" data-uk-toggle={'target: #modal-'+props.data.id} onClick={(e) => handleEditorSlide(props.data.id)}>
                            <i className="fas fa-code fa-sm uk-margin-small-right"></i>Edit HTML code
                        </button>
                        <button className="uk-button uk-button-default uk-border-rounded uk-margin-right" onClick={(e) => props.callback(e, props.data.id, props.parentData)}>
                            <i className="fas fa-trash-alt fa-sm uk-margin-small-right"></i>Delete slide
                        </button>
                        <div id={'modal-'+props.data.id} className="uk-modal-container uk-flex-top" data-uk-modal>
                            <div className="uk-modal-dialog uk-modal-body uk-margin-auto-vertical uk-border-rounded blockit-code-editor">
                                <div id={'editor-slide-'+props.data.id}></div>
